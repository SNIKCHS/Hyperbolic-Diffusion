...
class HGCLayer(nn.Module):
    def __init__(self, manifold, in_features, out_features, c_in, c_out, act):
        super().__init__()
        self.manifold = geoopt.Lorentz(learnable=True)
    ...
    def HypAgg(self, x, distances, edges, node_mask, edge_mask):
        ...
        geodesic = self.manifold.dist(x[row], x[col],keepdim=True)
        ...
        x_local_tangent = self.manifold.logmap(x[row], x[col])  # (b*n_node*n_node,dim)  x_col落在x_row的切空间
        ...
        support_t = self.manifold.proju(x, out)
