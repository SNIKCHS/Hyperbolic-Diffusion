...
from manifolds import ManifoldParameter, Hyperboloid,Euclidean
...
class CentroidDistance(nn.Module):
    ...
    def __init__(self,num_centroid,dim, manifold,c):
        ...
        # )
        geoopt.manifolds.PoincareBall.component_inner()
        self.centroid_embedding = ManifoldParameter(
            torch.zeros(num_centroid, dim),True,manifold,c
        )
    ...
    def forward(self, node_repr, mask):
        ...
        if self.args.manifold == 'Hyperboloid':
            centroid_repr = self.centroid_embedding.data
        else:
            centroid_repr = self.manifold.exp_map_zero(self.centroid_embedding)
        ...
        centroid_repr = centroid_repr.unsqueeze(0).expand(
                                                ...
                                                -1).contiguous().view(-1, self.args.dim)
        # get distance
        node_centroid_dist = torch.sqrt(self.manifold.sqdist(node_repr, centroid_repr))
